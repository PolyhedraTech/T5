...
class DiscoMecanico:
    def __init__(self, posicion=(0.0, 0.0, 0.0), radio_disco=0.5, radio_vortice=2.5, 
        ...
        self.es_rotado_90 = es_rotado_90
...
class SimuladorMultidiscoGUI(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Simulador Multidisco 3D - Análisis Volumétrico de Presión")
    ...
    def crear_panel_control(self):
        ...
        btn_simular.setStyleSheet("font-weight: bold; background-color: #007ACC; color: white; padding: 6px;")
        ...
        btn_simplify.setStyleSheet("font-weight: bold; background-color: #5CB85C; color: white; padding: 6px;")
        btn_simplify.clicked.connect(self.ejecutar_simplificacion)
        layout.addWidget(btn_simplify)

        btn_polyhedron = QPushButton("Fronteras / Polyhedron (Planos de Escape)")
        btn_polyhedron.setStyleSheet("font-weight: bold; background-color: #9B59B6; color: white; padding: 6px;")
        btn_polyhedron.clicked.connect(self.ejecutar_polyhedron)
        layout.addWidget(btn_polyhedron)

        btn_presion = QPushButton("Calcular Presión (Volumétrico 3D)")
        btn_presion.setStyleSheet("font-weight: bold; background-color: #E67E22; color: white; padding: 6px;")
        btn_presion.clicked.connect(self.ejecutar_calculo_presion)
        layout.addWidget(btn_presion)

        btn_limpiar = QPushButton("Limpiar Malla")
        btn_limpiar.setStyleSheet("font-weight: bold; background-color: #D9534F; color: white; padding: 6px;")
    ...
    def mostrar_solo_configuracion(self):
        ...
        if self.discos_malla_A:
            ...
            self.plotter.add_mesh(malla_A, color="silver", opacity=0.85)

        if self.discos_malla_B:
            malla_B = pv.MultiBlock([d.obtener_malla_geometria() for d in self.discos_malla_B]).combine()
            self.plotter.add_mesh(malla_B, color="orange", opacity=0.85)
    ...
    def ejecutar_simplificacion(self):
        ...
        info_str = (
            f"MODO SIMPLIFICADO ('SIMPLIFY'):\n"
            f"--------------------------------------------------\n"
            f"• Flechas Inter-vórtices: Vectores de movimiento resultante entre discos adyacentes.\n"
            f"• Tamaño y Color: Magnitud de la velocidad en el corredor inter-vórtice."
        )
        self.actualizar_texto_info(info_str)

    def ejecutar_polyhedron(self):
        """Calcula y muestra los planos de frontera y la estructura poliédrica de confinamiento."""
        self.mostrar_solo_configuracion()
        todos_los_discos = self.obtener_todos_los_discos()
        ...
        for i in range(n_discos):
            ...
            for idx_j in vecinos_idx:
                if i < idx_j:
                    ...
                    v_mag = np.linalg.norm([vx_tot, vy_tot, vz_tot])
                    if v_mag > 0.05:
                        puntos_puertas.append(p_medio)

        if len(puntos_puertas) >= 4:
            # Limpiar puntos duplicados o extremadamente cercanos para evitar triángulos degenerados
            cloud = pv.PolyData(np.array(puntos_puertas)).clean(tolerance=1e-4)
            
            # CORRECCIÓN: Especificar explicitamente el algoritmo para eliminar la advertencia
            poliedro = cloud.delaunay_3d().extract_surface(algorithm='dataset_surface')
            ...
            self.plotter.add_mesh(
                ...
                label="Puertas de Escape"
            )

            volumen = poliedro.volume
            area = poliedro.area
            n_caras = poliedro.n_cells
            n_puntos = poliedro.n_points

            info_str = (
                f"ESTRUCTURA POLIÉDRICA DE FRONTERAS:\n"
                f"--------------------------------------------------\n"
                f"• Planos (Caras): {n_caras} superficies semitransparentes de escape\n"
                f"• Puertas (Nodos): {n_puntos} puntos inter-vórtice activos\n"
                f"• Volumen de Confinamiento: {volumen:.2f} m³\n"
                f"• Área Superficial Total: {area:.2f} m²\n"
                f"• Geometría: Envolvente convexa de barreras cinemáticas"
            )
        else:
            info_str = "Se requieren al menos 4 puntos de interacción para construir la estructura poliédrica."

        self.actualizar_texto_info(info_str)

    def ejecutar_calculo_presion_01(self):
        """Calcula y muestra la ISOSUPERFICIE EN FORMA DE DODECAEDRO RÓMBICO (Fronteras de Presión)."""
        self.mostrar_solo_configuracion()
        todos_los_discos = self.obtener_todos_los_discos()
        ...
        # 1. Aumentar resolución para capturar las aristas afiladas del poliedro
        L = 50.0
        puntos_malla = 52
        x = np.linspace(-L/2, L/2, puntos_malla)
        y = np.linspace(-L/2, L/2, puntos_malla)
        ...
        rho = 1.225
        p_ref = 101325.0
        P_estatica = p_ref - 0.5 * rho * (V_mag ** 2)

        # 4. Creación de la Malla ImageData
        grid = pv.ImageData()
        grid.dimensions = (puntos_malla, puntos_malla, puntos_malla)
        grid.origin = (-L/2, -L/2, -L/2)
        grid.spacing = (L/(puntos_malla-1), L/(puntos_malla-1), L/(puntos_malla-1))
        ...
        if np.isclose(p_min, p_max):
            info_str = "Estado: Discos apagados. Presión uniforme."
            self.actualizar_texto_info(info_str)
            return

        # 5. ISOSUPERFICIES ESPECÍFICAS PARA REVELAR EL DODECAEDRO RÓMBICO
        # Se seleccionan 3 isosuperficies en el rango superior (alta presión / remanso inter-vórtice)
        val_cerca_max = p_max - 0.08 * (p_max - p_min)
        val_medio_alto = p_max - 0.25 * (p_max - p_min)
        val_núcleos    = p_min + 0.15 * (p_max - p_min)

        isosuperficies = grid.contour(
            isosurfaces=[val_núcleos, val_medio_alto, val_cerca_max], 
            scalars="Presion (Pa)"
        )

        # Renderizar la celda poliédrica con bordes/aristas visibles
        self.plotter.add_mesh(
            isosuperficies,
            scalars="Presion (Pa)",
            cmap="plasma",
            opacity=0.55,
            smooth_shading=True,
            show_edges=True,        # Muestra la malla de alambre/aristas del poliedro
            edge_color="cyan",
            line_width=1
        )

        self.plotter.add_mesh(grid.outline(), color="white", opacity=0.2)

        info_str = (
            f"GEOMETRÍA POLIÉDRICA (DODECAEDRO RÓMBICO / WIGNER-SEITZ):\n"
            f"--------------------------------------------------\n"
            f"• Presión Mínima: {p_min:.1f} Pa | Presión Máxima: {p_max:.1f} Pa\n"
            f"• Se muestran las barreras de colisión inter-vórtice (Remanso).\n"
            f"• NOTA TEÓRICA: Para maximizar las caras del Dodecaedro Rómbico,\n"
            f"  asegúrate de seleccionar la 'Estructura Tipo Sal' y ajustar R_v ≥ 3.5 m."
        )
        self.actualizar_texto_info(info_str)

    def ejecutar_calculo_presion_02(self):
        """Calcula y muestra ISOSUPERFICIES 3D de presión estática (Volumen / Poliedros de Isopresión)."""
        self.mostrar_solo_configuracion()
        todos_los_discos = self.obtener_todos_los_discos()
        ...
        # 1. Definir Dominio Tridimensional
        L = 50.0
        puntos_malla = 40
        x = np.linspace(-L/2, L/2, puntos_malla)
        y = np.linspace(-L/2, L/2, puntos_malla)
        ...
        # 3. Ecuación de Bernoulli para presión estática
        rho = 1.225
        p_ref = 101325.0
        P_estatica = p_ref - 0.5 * rho * (V_mag ** 2)

        # 4. Creación de la Malla (Ordenamiento Fortran 'F' para correcta alineación 3D)
        grid = pv.ImageData()
        grid.dimensions = (puntos_malla, puntos_malla, puntos_malla)
        grid.origin = (-L/2, -L/2, -L/2)
        grid.spacing = (L/(puntos_malla-1), L/(puntos_malla-1), L/(puntos_malla-1))
        ...
        # 5. Control de estado: Discos Apagados (Campos uniformes)
        if np.isclose(p_min, p_max):
            info_str = (
                f"CÁLCULO DE PRESIÓN ESTÁTICA:\n"
                f"--------------------------------------------------\n"
                f"• Estado: Todos los discos están apagados (Ω = 0).\n"
                f"• Presión uniforme en el dominio: {p_min:.1f} Pa\n"
                f"• No existen gradientes para generar figuras 3D."
            )
            self.actualizar_texto_info(info_str)
            return

        # 6. GENERACIÓN DE GEOMETRÍAS 3D (ISOSUPERFICIES / CONTOURS)
        # Extraemos 6 niveles isométricos de presión distribuidos entre el mín y máx
        n_niveles = 6
        niveles_presion = np.linspace(p_min + 0.05 * (p_max - p_min), p_max - 0.05 * (p_max - p_min), n_niveles)
        
        isosuperficies = grid.contour(isosurfaces=niveles_presion, scalars="Presion (Pa)")

        # Renderizar la estructura 3D resultante
        self.plotter.add_mesh(
            isosuperficies,
            scalars="Presion (Pa)",
            cmap="turbo",
            opacity=0.65,              # Transparencia para ver estructuras internas
            smooth_shading=True,       # Renderizado suave de caras 3D
            show_edges=False
        )

        # Delimitador del volumen de cálculo
        self.plotter.add_mesh(grid.outline(), color="white", opacity=0.2)

        info_str = (
            f"FIGURA 3D DE ISOSUPERFICIES DE PRESIÓN:\n"
            f"--------------------------------------------------\n"
            f"• Presión Mínima (Centros de Vórtice): {p_min:.1f} Pa\n"
            f"• Presión Máxima (Barreras/Paredes): {p_max:.1f} Pa\n"
            f"• Niveles 3D Representados: {n_niveles} envolturas cerradas\n"
            f"• Estructura: Geometría tridimensional continua del campo de presión."
        )
        self.actualizar_texto_info(info_str)

    def ejecutar_calculo_presion(self):
        """Calcula y aísla la FRONTERA DE MÁXIMA PRESIÓN (Límites de Remanso Inter-vórtice)."""
        self.mostrar_solo_configuracion()
        todos_los_discos = self.obtener_todos_los_discos()
        ...
        # 1. Aumento de resolución espacial para aristas afiladas inter-celda
        L = 50.0
        puntos_malla = 52
        x = np.linspace(-L/2, L/2, puntos_malla)
        y = np.linspace(-L/2, L/2, puntos_malla)
        ...
        # 3. Bernoulli: P_estatica = P_ref - 0.5 * rho * V^2
        rho = 1.225
        p_ref = 101325.0
        P_estatica = p_ref - 0.5 * rho * (V_mag ** 2)

        # 4. Construcción del volumen
        grid = pv.ImageData()
        grid.dimensions = (puntos_malla, puntos_malla, puntos_malla)
        grid.origin = (-L/2, -L/2, -L/2)
        grid.spacing = (L/(puntos_malla-1), L/(puntos_malla-1), L/(puntos_malla-1))
        ...
        if np.isclose(p_min, p_max):
            self.actualizar_texto_info("Estado: Discos apagados. Campo de presión uniforme.")
            return

        # 5. AISLAMIENTO EXCLUSIVO DE MÁXIMOS DE PRESIÓN
        # Se capturan dos umbrales en el extremo superior (92% y 98% de la presión estática máxima)
        dp = p_max - p_min
        iso_alta_1 = p_max - 0.08 * dp
        iso_alta_2 = p_max - 0.02 * dp

        isosuperficies = grid.contour(
            isosurfaces=[iso_alta_1, iso_alta_2], 
            scalars="Presion (Pa)"
        )

        # Renderizado de caras límite y aristas de colisión
        self.plotter.add_mesh(
            isosuperficies,
            scalars="Presion (Pa)",
            cmap="YlOrRd",
        ...
        self.plotter.add_mesh(grid.outline(), color="white", opacity=0.15)

        info_str = (
            f"FRONTERA LÍMITE DE MÁXIMA PRESIÓN:\n"
            f"--------------------------------------------------\n"
            f"• Presión Máxima ($P_{{max}}$): {p_max:.1f} Pa\n"
            f"• Umbrales Visibles: {iso_alta_1:.1f} Pa a {iso_alta_2:.1f} Pa\n"
            f"• Estructura: Geometría de paredes/planos de choque inter-vórtices.\n"
            f"• Sugerencia: Usa 'Estructura Tipo Sal' y Ω_A = Ω_B para simetría completa."
        )
        self.actualizar_texto_info(info_str)
