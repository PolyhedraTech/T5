...
class SimuladorMultidiscoGUI(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Simulador Multidisco 3D - Análisis Poliédrico de Fronteras")
    ...
    def crear_panel_control(self):
        ...
        # BOTÓN NUEVO: FRONTERAS / POLYHEDRON
    ...
    def ejecutar_polyhedron(self):
        ...
        # Identificar las posiciones inter-vórtice (puertas de paso)
        for i in range(n_discos):
            pos_i = pos_array[i]
            distancias = np.linalg.norm(pos_array - pos_i, axis=1)
            vecinos_idx = np.where((distancias > 0.1) & (distancias <= 12.0))[0]
        ...
        if len(puntos_puertas) >= 4:
            cloud = pv.PolyData(np.array(puntos_puertas))
            
            # Generar el Poliedro de Confinamiento (Envolvente Convexa de los nodos de escape)
            poliedro = cloud.delaunay_3d().extract_surface()

            # Renderizar caras semitransparentes (planos de escape)
            self.plotter.add_mesh(
                poliedro,
                color="#8E44AD",
                opacity=0.40,
            ...
            # Renderizar los nodos/puertas de salida
            self.plotter.add_mesh(
                cloud,
                color="yellow",
                point_size=10.0,
            ...
            # Métrica y Análisis Estructural
