...
class SimuladorMultidiscoGUI(QMainWindow):
    ...
    def crear_panel_control(self):
        ...
        self.check_100m = QCheckBox("Dominio Extendido (100x100x100 m)")
        self.check_100m.stateChanged.connect(self.cambiar_configuracion)
        layout.addWidget(self.check_100m)
    ...
    def generar_archivo_rejilla_regular(self):
        paso = 10.0
        limite = 45.0 if hasattr(self, 'check_100m') and self.check_100m.isChecked() else 20.0
        rango = np.arange(-limite, limite + paso, paso)
        posiciones = []
        for x in rango:
            for y in rango:
                for z in rango:
    ...
    def generar_archivo_estructura_sal(self):
        ...
        limite_x = limite + 5.0
        posiciones = []
        y_niveles = np.arange(-limite, limite + paso, paso)
        z_niveles = np.arange(-limite, limite + paso, paso)

        for j, y in enumerate(y_niveles):
            for k, z in enumerate(z_niveles):
                shift = desfase if (j + k) % 2 != 0 else 0.0
                x_fila = np.arange(-limite, limite + paso, paso) + shift
                for x in x_fila:
                    if -limite_x <= x <= limite_x:
                        posiciones.append((x, y, z))
    ...
    def instanciar_discos(self):
        ...
        limite_b = 49.0 if self.check_100m.isChecked() else 24.0
        ...
        self.discos_malla_B = [
            ...
            for pos in posiciones_base
            if np.all(np.abs(np.array(pos) + offset) <= limite_b)
        ]
    ...
    def mostrar_solo_configuracion(self):
        ...
        limite_caja = 50.0 if self.check_100m.isChecked() else 25.0
        caja = pv.Box(bounds=[-limite_caja, limite_caja, -limite_caja, limite_caja, -limite_caja, limite_caja])
        ...
        info_str = (
            ...
            f"Dominio: {'100x100x100 m' if self.check_100m.isChecked() else '50x50x50 m'}\n"
    ...
    def ejecutar_simulacion(self):
        ...
        L = 100.0 if self.check_100m.isChecked() else 50.0
        puntos_malla = 32
    ...
    def ejecutar_calculo_presion(self):
        """Calcula y aísla las ISOSUPERFICIES 3D de máxima presión estática (Fronteras de Remanso)."""
        self.mostrar_solo_configuracion()
        todos_los_discos = self.obtener_todos_los_discos()
        ...
        info_str = (
            ...
            f"--------------------------------------------------\n"
            f"• Dominio Activo: {'100x100x100 m' if self.check_100m.isChecked() else '50x50x50 m'}\n"
