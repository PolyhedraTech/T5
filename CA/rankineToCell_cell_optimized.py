...
class SimuladorMultidiscoGUI(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Simulador Multidisco 3D - Vista Simplificada por Vectores")
    ...
    def crear_panel_control(self):
        ...
        layout.addWidget(QLabel("Líneas de Flujo por Disco:"))
        self.spin_lineas_disco = QSpinBox()
        self.spin_lineas_disco.setRange(1, 12)
        self.spin_lineas_disco.setValue(2)
        layout.addWidget(self.spin_lineas_disco)

        layout.addWidget(QLabel("Radio de Tubos (Espesor):"))
        ...
        layout.addWidget(self.spin_radio_tubo)
        ...
        btn_simular = QPushButton("Calcular Simulación")
        ...
        btn_simplify.setStyleSheet("font-weight: bold; background-color: #5CB85C; color: white; padding: 10px;")
        btn_simplify.clicked.connect(self.ejecutar_simplificacion)
        layout.addWidget(btn_simplify)

        btn_limpiar = QPushButton("Limpiar Líneas")
        btn_limpiar.setStyleSheet("font-weight: bold; background-color: #D9534F; color: white; padding: 10px;")
    ...
    def mostrar_solo_configuracion(self):
        ...
        if self.discos:
            mallas_discos = [disco.obtener_malla_geometria() for disco in self.discos]
            malla_bloque_discos = pv.MultiBlock(mallas_discos).combine()
            self.plotter.add_mesh(malla_bloque_discos, color="silver", opacity=0.8)

        self.plotter.add_axes(line_width=2)
        
        info_str = (
            f"Configuración: {self.combo_distribucion.currentText()}\n"
            f"Discos Totales: {len(self.discos)}\n"
            f"Estado: [Sin simulación activa]\n"
            f"Pulse 'Calcular Simulación' o 'Simplify'."
        )
    ...
    def ejecutar_simulacion(self):
            ...
            if disco.sentido_sup != 0:
                for th in angulos:
                    semillas_list.append([
                        disco.posicion[0] + r_muestreo * np.cos(th),
                        disco.posicion[1] + r_muestreo * np.sin(th),
                        disco.posicion[2] + 1.0
                    ])
        ...
        info_str = (
            f"Configuración: {self.combo_distribucion.currentText()}\n"
            f"Discos Activos: {len(self.discos)} | Muestreo: {lineas_por_disco} líneas/disco\n"
            f"--------------------------------------------------\n"
            f"LEYENDA DE LÍNEAS DE CORRIENTE:\n"
            f"• Trayectoria: Muestra el recorrido continuo del fluido en el espacio 3D.\n"
            f"• Deformación: Representa la interferencia vectorial V_total = SUM(V_i).\n"
            f"• Escala Color: Azul (baja velocidad) -> Rojo (alta velocidad)."
        )
        self.actualizar_texto_info(info_str)

    def ejecutar_simplificacion(self):
        """Modo 'SIMPLIFY': Traza una única flecha direccional entre vórtices adyacentes."""
        self.mostrar_solo_configuracion()
        ...
        pos_array = np.array([disco.posicion for disco in self.discos])
        n_discos = len(self.discos)

        # Buscar vecinos cercanos (distancia menor a 14 metros)
        for i in range(n_discos):
            pos_i = pos_array[i]
            # CORREGIDO: np.linalg.norm
            distancias = np.linalg.norm(pos_array - pos_i, axis=1)
            
            vecinos_idx = np.where((distancias > 0.1) & (distancias <= 14.0))[0]
        ...
        if puntos_origen:
            ...
            # Crear glifos de flechas orientadas
        ...
        info_str = (
            ...
            f"--------------------------------------------------\n"
            f"• Flecha Única Inter-Vórtice: Ubicada en el centro geométrico entre discos vecinos.\n"
            f"• Dirección de Flecha: Indica hacia dónde se movería una partícula en esa región.\n"
            f"• Tamaño y Color de Flecha: Proporcionales a la magnitud de velocidad inducida."
        )
        self.actualizar_texto_info(info_str)
